...
main :: proc() {
	fmt.println("joltc hinge constraint smoke test")
	...
	settings_b.Position = rvec3(0, 4, 0)
	...
	if body_a == nil || body_b == nil {
		fmt.eprintln("failed to create hinge bodies")
		os.exit(1)
	}
	...
	hinge_settings: joltc.Hinge_Constraint_Settings
	joltc.JPC_HingeConstraintSettings_default(&hinge_settings)
	hinge_settings.Space = .WORLD_SPACE
	hinge_settings.Point1 = rvec3(0, 3, 0)
	hinge_settings.Point2 = rvec3(0, 3, 0)
	hinge_settings.HingeAxis1 = vec3(0, 0, 1)
	hinge_settings.HingeAxis2 = vec3(0, 0, 1)
	hinge_settings.NormalAxis1 = vec3(1, 0, 0)
	hinge_settings.NormalAxis2 = vec3(1, 0, 0)

	constraint := joltc.JPC_HingeConstraintSettings_Create(&hinge_settings, body_a, body_b)
	if constraint == nil {
		fmt.eprintln("JPC_HingeConstraintSettings_Create failed")
		os.exit(1)
	}

	joltc.JPC_PhysicsSystem_AddConstraint(physics, cast(^joltc.Constraint)constraint)

	joltc.JPC_HingeConstraint_SetMotorState(constraint, .VELOCITY)
	joltc.JPC_HingeConstraint_SetTargetAngularVelocity(constraint, 2.5)
	motor_state := joltc.JPC_HingeConstraint_GetMotorState(constraint)
	target_vel := joltc.JPC_HingeConstraint_GetTargetAngularVelocity(constraint)
	...
	lambda_pos := joltc.JPC_HingeConstraint_GetTotalLambdaPosition(constraint)
	lambda_rot := joltc.JPC_HingeConstraint_GetTotalLambdaRotation(constraint)
	lambda_motor := joltc.JPC_HingeConstraint_GetTotalLambdaMotor(constraint)

	fmt.printf("motor state   = %v\n", motor_state)
	fmt.printf("target vel    = %v\n", target_vel)
	fmt.printf("lambda pos    = (%v, %v, %v)\n", lambda_pos.x, lambda_pos.y, lambda_pos.z)
	fmt.printf("lambda rot    = (%v, %v)\n", lambda_rot.x, lambda_rot.y)
	fmt.printf("lambda motor  = %v\n", lambda_motor)

	if motor_state != joltc.Motor_State.VELOCITY {
		fmt.eprintln("hinge motor state roundtrip failed")
		os.exit(1)
	}

	if target_vel != 2.5 {
		fmt.eprintf("hinge target angular velocity mismatch: %v\n", target_vel)
		os.exit(1)
	}

	fmt.println("hinge constraint smoke test passed")
