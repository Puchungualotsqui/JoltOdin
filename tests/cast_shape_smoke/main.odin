...
cast_reset :: proc "c" (self: rawptr) {
	s := cast(^Cast_State)self
	s.hit_count = 0
	s.best_fraction = 1e30
	s.last_body = joltc.BodyID(0)
}

cast_add_hit :: proc "c" (
	self: rawptr,
	base: ^joltc.CastShapeCollector,
	result: ^joltc.Shape_Cast_Result,
) {
	s := cast(^Cast_State)self
	s.hit_count += 1
	if result.Fraction < s.best_fraction {
		s.best_fraction = result.Fraction
		s.last_body = result.BodyID2
	}
	joltc.JPC_CastShapeCollector_UpdateEarlyOutFraction(base, result.Fraction)
}
...
main :: proc() {
	fmt.println("joltc cast shape smoke test")
	...
	cast_shape := create_box_shape(vec3(0.5, 0.5, 0.5))
	...
	target_settings.ObjectLayer = NON_MOVING_LAYER
	...
	fmt.printf("target added  = %v\n", joltc.JPC_BodyInterface_IsAdded(body_interface, target_id))

	// Step once so broadphase/query state is settled.
	...
	collector := joltc.JPC_CastShapeCollector_new(&state, collector_fns)

	args: joltc.Narrow_Phase_Query_Cast_Shape_Args
...
args.ShapeCast.Direction = vec3(0, -10, 0)
	...
	joltc.JPC_NarrowPhaseQuery_CastShape(query, &args)

	fmt.printf("hit count     = %v\n", state.hit_count)
	fmt.printf("best fraction = %v\n", state.best_fraction)
	fmt.printf("body id       = %v\n", state.last_body)

	if state.hit_count < 1 {
		fmt.eprintln("expected at least one cast-shape hit")
		os.exit(1)
	}

	if !(state.best_fraction > 0 && state.best_fraction <= 1) {
		fmt.eprintf("cast fraction out of range: %v\n", state.best_fraction)
		os.exit(1)
	}

	if state.last_body != target_id {
		fmt.eprintf("cast hit wrong body: got %v expected %v\n", state.last_body, target_id)
		os.exit(1)
	}

	fmt.println("cast shape smoke test passed")

	joltc.JPC_CastShapeCollector_delete(collector)
	joltc.JPC_BodyInterface_RemoveBody(body_interface, target_id)
	joltc.JPC_BodyInterface_DestroyBody(body_interface, target_id)
	joltc.JPC_Shape_Release(cast_shape)
